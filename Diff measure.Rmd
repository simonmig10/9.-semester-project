...
Ideer til at volatilitet påvirker efterspørgslen efter aktier:

1. Økonometrisk estimerer effekten og direkte lave et link 

2. kigge på afstanden mellem krævet afkast og det aktuelle afkast sidste periode, jo større afstand jo større negativ effekt. 

- Når jeg laver re_re ved jeg ik om det er et problem at initial værdien for aktier starter i 1 med stready state i 1.2, automatisk stigning. 
- Har løst problemet ved at indsætte en "vent" variable som først går at effekten kommer ind efter jeg har lave stød, så modellen har tilpasset sig

- Har i denne forsøgt at bruge afstannden mellem der reale afkast og det "krævede" afkast og derigennem lavet en påvirkning på det forventede afkast. 
men så bliver effekten bare en anden da det så er den lambda koefficien der ganges på for når afkastet falder --> giver en anden historie.
...
er_re ~ er_rbl + vent*vol_eff*diff_er + (1-vent)*beta_mu, 
...
beta1 ~ er_re/100,
####Volatility
g ~ g_in + vol*g_bar, ##Ny govement spending
g_bar ~ vol_1*g_bar[-1] + (g_bar_var^0.5)*varepsilon, #AR(1) process
g_bar_var ~ a_0 + a_1*g_bar_var[-1]*(varepsilon[-1]^2) + b_1*g_bar_var[-1], #Brugt en GARCH(1,1) process, men er lidt i tvivl om den er skrevet rigtigt --> se noter
####Excess return on stocks
ex_e ~ mu_ex + sd_ex*u_t,
mu_ex ~ beta_mu + delta_mu*g_bar_var,
re_re ~ (p_e - p_e[-1])/p_e[-1]*100, 
diff_er ~ ex_e - re_re 
)
```
...
er_rbl ~ 1,
g_bar ~ 5,
g_bar_var ~1, #sætter ligmed 1 så den følger sd
...
```{r}
plot(sim$re_re, type="l",xlab="Year",
     ylab="Share",lty=1,lwd=2, col="black"); grid()
```

```{r}
plot(sim2$diff_er, type="l",xlab="Year",
     ylab="Share",lty=1,lwd=2, col="black");grid()
```
```{r}
plot(sim2$e_d, type="l",xlab="Year",
     ylab="Share",lty=1,lwd=2, col="black");grid()
```
