...
- Tilføjet lambda25 og lambda35. 

- Så længer jeg ikk bruger re_re er det ligegyldigt om det kommer med i chock eller ikke! Så kan faktisk bare fjerne "vent_1" variablen. 

- Har brugt variance i "G" som uncertainty measure som går direke ind i akieprisen, gennem et eksra led i tobins portfolie teori. 
...
p_e ~ v_e*(lambda20 + lambda22*er_re + lambda23*er_rbl + lambda24*(yd_e/v_e) + lambda25* g_bar_var*vent_1)/e_s, 
e_d ~ v_e*(lambda20 + lambda22*er_re + lambda23*er_rbl + lambda24*(yd_e/v_e) + lambda25* g_bar_var*vent_1)/p_e, 
####Eftersp�rgsel efter bonds - eq. 5.11
bl_d ~ v_e*(lambda30 + lambda32*er_re + lambda33*er_rbl + lambda34*(yd_e/v_e) + lambda35* g_bar_var*vent_1)/p_bl,
####bl_h - eq. 5.13
bl_h ~ bl_d,
h_h ~ h_d,
...
lambda33 ~ 0.0183,
lambda34 ~ -0.015,
lambda25 ~ -0.001, #denne er tilføjet
vent_1 ~ 0, #Denne er tilføjet
lambda35 ~ 0.001, # Denne er ilføjet
theta ~ 0.4,
theta1 ~ 0.27,
gamma ~ 0.1, 
kappa ~ 1,
...
```{r}
plot(sim$g_bar, type="l",xlab="Year",
     ylab="Share",lty=1,lwd=2, col="black"); grid()
```

```{r}
plot(sim$g_bar_var, type="l",xlab="Year",
     ylab="Share",lty=1,lwd=2, col="black");grid()
```
```{r}
